...
def _tensor_to_temp_png(tensor) -> Path:
    ...
    os.close(fd)
    debugprint(f"Image Save Path: {path}")
...
def _write_temp_text_file(prefix: str, text: str) -> Path:
    ...
    os.close(fd)
    text_path = Path(path)
    debugprint(f"Temporary Text File Save Path: {path}")
...
def split_extra_args(extra_args: str) -> list[str]:
    ...
    argparts = [part.strip("\"'") for part in parts]
    debugprint(f"Extra Arguments: {argparts}")
    return argparts

def normalize_llama_seed(seed: int) -> int:
    seed = int(seed)
    if seed == LLAMA_RANDOM_SEED:
        debugprint(f"Seed: {str(seed)}")
        return LLAMA_RANDOM_SEED
    if 0 <= seed <= MAX_LLAMA_SEED:
        debugprint(f"Seed <3 MAX_LLAMA_SEED: {str(seed)}")
        return seed
...
def build_command(
    ...
    if image is not None:
        ...
        debugprint(f"Image Path: {image_paths}")
        cleanup_paths.extend(image_paths)

    prompt_path = _write_prompt_file(prompt)
    debugprint(f"Prompt Path: {prompt_path}")
